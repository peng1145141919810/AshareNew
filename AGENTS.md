...
Before making changes in this repository, read `F:\quant_data\AshareC#\CODEX_DEV_LOG.md`.
...
- This Rider workspace currently mirrors code first. Large `data/`, training tables, and many runtime artifacts still remain under `F:\quant_data\Ashare\data` unless the user explicitly migrates them.
- Treat `F:\quant_data\Ashare` as a read-only external dependency unless the user explicitly says otherwise.
- Do not modify files in the old live Python repo. New work must land in `F:\quant_data\AshareC#`.
...
- Watcher control scripts:
  - start: `powershell -ExecutionPolicy Bypass -File F:\quant_data\AshareC#\scripts\start_codex_dev_log_sync.ps1`
  - stop: `powershell -ExecutionPolicy Bypass -File F:\quant_data\AshareC#\scripts\stop_codex_dev_log_sync.ps1`
- Logon autostart installer:
  - install: `powershell -ExecutionPolicy Bypass -File F:\quant_data\AshareC#\scripts\install_codex_dev_log_sync_autostart.ps1`
  - remove: `powershell -ExecutionPolicy Bypass -File F:\quant_data\AshareC#\scripts\remove_codex_dev_log_sync_autostart.ps1`
...
## Dev Log Maintenance
- `F:\quant_data\AshareC#\CODEX_DEV_LOG.md` is a living handoff file, not a one-time snapshot.
...
- Wrapped business root mirror: `F:\quant_data\AshareC#\main_research_runner.py`
- The live operator repo can still remain `F:\quant_data\Ashare`; confirm the local dev log before assuming this copy is standalone.
- The old live repo is operationally protected and read-only from this workspace's perspective.
...
## Runtime Notes
- This repo is currently the Rider / C# migration workspace copy, not an already cut-over live runtime root.
- Large runtime data, train tables, and many generated artifacts still live in the original `F:\quant_data\Ashare\data` tree unless explicitly migrated.
